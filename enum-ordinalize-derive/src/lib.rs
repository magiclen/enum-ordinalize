...
use syn::{
    ...
    spanned::Spanned,
    Attribute, Data, DeriveInput, Expr, Fields, Ident, Lit, Meta, MetaList, Token, UnOp, Variant,
    Visibility,
};
...
fn get_variant_type_from_attrs(attrs: &[Attribute]) -> Result<VariantType, syn::Error> {
...
fn handle_discriminants<'a>(
    name: &'a Ident,
    variants: &'a Punctuated<Variant, Token![,]>,
    variant_type: &mut VariantType,
) -> Result<(Vec<BigIntWrapper<'a>>, Vec<&'a Ident>, bool), syn::Error> {
                    ...
                    let value = if let Some((_, exp)) = variant.discriminant.as_ref() {
                        match exp {
                            ...
                            Expr::Path(_) | Expr::Cast(_) | Expr::Binary(_) | Expr::Call(_) => {
                                return Err(panic::constant_variable_on_non_determined_size_enum(
                                    exp.span(),
                                ))
                            },
    ...
    Ok((values, variant_idents, use_constant_counter))
}
...
pub fn ordinalize_derive(input: TokenStream) -> TokenStream {
    ...
    impl Parse for MyDeriveInput {
        fn parse(input: ParseStream) -> Result<Self, syn::Error> {
            ...
            match &ast.data {
                Data::Enum(data) => {
                    let mut variant_type = get_variant_type_from_attrs(&ast.attrs)?;
                    ...
                    for attr in ast.attrs.iter() {
                        ...
                        if path.is_ident("ordinalize") {
                            if let Meta::List(list) = &attr.meta {
                                let result = list.parse_args_with(
                                    Punctuated::<MetaList, Token![,]>::parse_terminated,
                                )?;
                    ...
                    let (values, variant_idents, use_constant_counter) =
                        handle_discriminants(name, &data.variants, &mut variant_type)?;
                ...
                _ => Err(panic::not_enum(name.span())),
            }
        }
    }
...
pub fn variants_derive(input: TokenStream) -> TokenStream {
    ...
    // Build the code
    let expanded = quote! {
        impl #impl_generics Variants for #name #ty_generics #where_clause {
            type VariantType = #variant_type;
